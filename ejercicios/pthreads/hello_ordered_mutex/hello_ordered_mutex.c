...
int main (int argc, char*argv[])
{
	...
	shared_data->mutex_collection = (pthread_mutex_t*) calloc(shared_data->thread_count, sizeof(pthread_mutex_t));
	if(shared_data->mutex_collection == NULL)
	{
	...
	{
		pthread_mutex_init(&shared_data->mutex_collection[index],/*attr*/NULL);
		pthread_mutex_lock(&shared_data->mutex_collection[index]);
	}
	
	pthread_mutex_unlock(&shared_data->mutex_collection[0]);
	...
	for(size_t index = 0; index < shared_data->thread_count; ++index)
	{
		pthread_mutex_destroy(&shared_data->mutex_collection[index]);
	}
	
	free(shared_data->mutex_collection);
...
void* run(void* data)
{
	...
	pthread_mutex_lock(&shared_data->mutex_collection[thread_num]);
	printf("Hellow World from secondary thread %zu of %zu\n", thread_num, thread_count); 
	pthread_mutex_unlock(&shared_data->mutex_collection[thread_num]);
	if(thread_num != thread_count-1)
	{
		pthread_mutex_unlock(&shared_data->mutex_collection[thread_num+1]);	
	}	
